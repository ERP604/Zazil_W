...
const Forum: React.FC = () => {
  ...
  const confirmResponder = async () => {
    if (preguntaToRespond && responseText.trim() !== '') {
      ...
      const id_usuario_respuesta = 1; // TODO: Reemplazar con el ID del usuario autenticado
          ...
          <div className="modal">
            ...
            <div className="modal-buttons">
              <button className="table-button" onClick={confirmResponder} disabled={loadingIds.includes(preguntaToRespond.id_pregunta)}>
                {loadingIds.includes(preguntaToRespond.id_pregunta) ? 'Procesando...' : 'Enviar Respuesta'}
              </button>
