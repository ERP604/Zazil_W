...
async function updateProduct(req, res) {
    ...
    getConnection((err, connection) => {
        ...
        connection.query(updateQuery, [nombre_producto, categoria, descripcion, precio, stock, id], (error, result) => {
            if (error) {
                connection.release();
...
async function updateImage(req, res) {
    ...
    getConnection((err, connection) => {
      ...
      // Consulta la ruta de la imagen actual
        connection.query('SELECT ruta_img FROM product_z WHERE id_producto = ?', [id], (error, results) => {
        if (error) {
            connection.release();
            return res.status(500).send({ alert: 'Error al consultar la base de datos' });
        }
        ...
        const oldImagePath = path.join(__dirname, '../uploads', results[0].ruta_img);
        ...
        const updateQuery = 'UPDATE product_z SET ruta_img = ? WHERE id_producto = ?';
        connection.query(updateQuery, [ruta_img, id], (error) => {
            connection.release();
            if (error) {
                return res.status(500).send({ alert: 'Error al actualizar la imagen del producto' });
            }
            res.status(200).send({ status: 'Imagen del producto actualizada correctamente' });
            });
        });
    });
}  
