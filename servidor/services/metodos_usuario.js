...
async function login(req, res) {
    ...
    connection.query(query, [email], (error, results) => {
        ...
        res.json({ status: true, id: user.id_usuario})
...
async function updateuserstatus(req, res) {
    ...
    getConnection((err, connection) => {
        ...
        connection.query(query, [estatus, id], (error, results) => {
            ...
            return res.status(200).send({ message: 'Estatus actualizado correctamente' });
        });
    });
}
...
async function updateUser(req, res) {
    ...
    const { nombre, apellido_paterno, apellido_materno, f_nacimiento, tipo_usuario, email } = req.body;

    // Validar que se hayan proporcionado los datos necesarios
    if (!nombre || !apellido_paterno || !apellido_materno || !f_nacimiento || !tipo_usuario || !email) {
        return res.status(400).send({ alert: 'Faltan datos' });
    }
    ...
    getConnection((err, connection) => {
        ...
        const values = [nombre, apellido_paterno, apellido_materno, f_nacimiento, tipo_usuario, email, id];

        connection.query(updateQuery, values, (error, result) => {
            if (error) {
                connection.release();
                return res.status(500).send({ alert: 'Error al actualizar el usuario' });
            }
...
async function updateImageUser(req, res) {
    ...
    const ruta_img = req.file ? req.file.filename : null;  // Guarda la ruta de la imagen
    console.log(req.body);
    // Validar que se haya proporcionado la imagen
    if (!ruta_img) {
    ...
    getConnection((err, connection) => {
        ...
        //Consulta de la ruta de la imagen actual 
            connection.query('SELECT ruta_img FROM usuario WHERE id_usuario = ?', [id], (error, results) => {
            if (error) {
                connection.release();
                return res.status(500).send({ alert: 'Error al obtener la imagen del usuario' });
            }
            if (results.length === 0) {
                connection.release();
                return res.status(404).send({ alert: 'Usuario no encontrado' });
            }
            const oldImagePath = path.join(__dirname, '../uploads_profile', results[0].ruta_img);
        ...
        // Preparar los valores para la consulta
        const updateQuery = 'UPDATE usuario SET ruta_img = ? WHERE id_usuario = ?';
        connection.query(updateQuery, [ruta_img, id], (error) => {
            connection.release();
            if (error) {   
                return res.status(500).send({ alert: 'Error al actualizar la imagen del usuario' });
            }
...
async function searchusers(req, res) {
    ...
    let query = "SELECT id_usuario, nombre, apellido_paterno, apellido_materno, estatus, tipo_usuario, email, ruta_img FROM usuario";
    let conditions = [];
    let params = [];

    // Búsqueda por nombre o email
    if (search) {
        conditions.push("(nombre LIKE ? OR apellido_paterno LIKE ? OR apellido_materno LIKE ? OR email LIKE ?)");
