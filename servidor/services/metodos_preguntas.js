...
async function addRespuesta(req, res) {
  ...
  if (!id_pregunta || !id_usuario_respuesta || !respuesta) {
    return res
      .status(400)
      .send({ alert: "Faltan datos necesarios para agregar la respuesta" });
  }
  ...
  getConnection((err, connection) => {
    ...
    connection.query(checkQuery, [id_pregunta], (error, results) => {
      if (error) {
        ...
        return res
          .status(500)
          .send({ alert: "Error al verificar la pregunta" });
      }
      ...
      const updateQuery = "UPDATE pregunta SET id_usuario_respuesta = ?, respuesta = ?, fecha_respuesta = ? WHERE id_pregunta = ?";
    
      connection.query(
        updateQuery,
        [id_usuario_respuesta, respuesta, fecha_respuesta, id_pregunta],
        (updateError, updateResult) => {
          ...
          if (updateError) {
            ...
            return res
              .status(500)
              .send({ alert: "Error al agregar la respuesta" });
          }
...
async function addPregunta(req, res) {
  ...
  if (!id_usuario || !titulo) {
    return res
      .status(400)
      .send({ alert: "Faltan datos necesarios para crear la pregunta" });
  }
  ...
  getConnection((err, connection) => {
    ...
    const insertQuery = `
            INSERT INTO pregunta (id_usuario, titulo, fecha_creacion)
            VALUES (?, ?, ?)
        `;
    connection.query(
      insertQuery,
      [id_usuario, titulo, fecha_creacion],
      (error, results) => {
        connection.release();
        if (error) {
          console.error("Error al insertar la pregunta:", error);
          return res.status(500).send({ alert: "Error al crear la pregunta" });
        }

        res.status(201).send({
          status: "Pregunta creada correctamente",
